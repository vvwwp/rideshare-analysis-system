...
class 对比分析系统:
    ...
    def calculate_balance_factors(self, n_indicators):
        ...
        for i in range(n_indicators):
            ...
            try:
                ...
                mean_score = np.mean(y_samples)
                integral_means.append(mean_score)
        
        # === 计算平衡因子 ===
        target_mean = 50.0  # 理想的积分均值（中性点）
        balance_factors = np.array([target_mean / mean_score if mean_score > 0 else 1.0 
    ...
    def calculate_entropy_weights(self, score_matrix):
        ...
        # === 第二层：积分均值平衡因子修正 ===
        balance_factors, integral_means, target_mean = self.calculate_balance_factors(n_indicators)
...
class 乘车体验对比分析GUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("🚗 乘车体验对比分析系统 v2.0 - 积分均值版")
    ...
    def create_header(self, parent):
        ...
        # 主标题
        title_label = tk.Label(header, text="🚗 乘车体验对比分析系统 - 积分均值版", 
                              font=('微软雅黑', 24, 'bold'), fg='white', bg='#2c3e50')
        title_label.pack(pady=10)
        
        # 副标题：算法说明
        subtitle_label = tk.Label(header, text="基于评分函数积分均值的客观平衡因子算法", 
