...
class 对比分析系统:
    ...
    def calculate_balance_factors(self, n_indicators):
        """计算平衡因子（用于修正科学标准中的隐性权重偏差）"""
        # 各指标的敏感度范围（基于评分函数分析）
        sensitivity_ranges = {
            0: 100,  # 平顺性: 0-100分，敏感度100
            1: 95,   # 稳定性: 5-100分，敏感度95
            2: 95,   # 噪声: 5-100分，敏感度95
            3: 95,   # 时间效率: 5-100分，敏感度95
            4: 100   # 价格: 0-100分，敏感度100（如果存在）
        }
        
        # 计算标准敏感度（平均值）
        current_ranges = [sensitivity_ranges.get(i, 95) for i in range(n_indicators)]
        standard_sensitivity = np.mean(current_ranges)
        
        # 计算平衡因子：标准敏感度 / 当前敏感度
        balance_factors = np.array([standard_sensitivity / range_val for range_val in current_ranges])
        
        return balance_factors, current_ranges, standard_sensitivity
    ...
    def calculate_entropy_weights(self, score_matrix):
        ...
        # === 第二层：平衡因子修正 ===
        balance_factors, sensitivity_ranges, standard_sensitivity = self.calculate_balance_factors(n_indicators)
...
class 乘车体验对比分析GUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("🚗 乘车体验对比分析系统 v2.1")
    ...
    def create_header(self, parent):
        ...
        # 主标题
        title_label = tk.Label(header, text="🚗 乘车体验对比分析系统", 
                              font=('微软雅黑', 28, 'bold'), fg='white', bg='#2c3e50')
